...
def plot_excursion_sets(exc_sets):
    if len(sorted(exc_sets.items())[0][1][1]) == 2:
        for soft, (mask_file, (exc_set_file, exc_set_file_neg), stat_file) in sorted(exc_sets.items()):
            ...
            display = plotting.plot_stat_map(to_display, display_mode='x', cut_coords=[4, 32], draw_cross=False, colorbar=True, title=soft.upper(), threshold=0.000001, vmax=4.2)
            display = plotting.plot_stat_map(to_display, cut_coords=[38], draw_cross=False, display_mode='y', threshold=0.000001, colorbar=False, vmax=4.2)
            # Additional plot: slices along z
            display = plotting.plot_stat_map(to_display, cut_coords=[-32, -18, 0, 12, 24, 40, 58], draw_cross=False, display_mode='z', threshold=0.000001, colorbar=False, vmax=4.2, title=soft.upper())
