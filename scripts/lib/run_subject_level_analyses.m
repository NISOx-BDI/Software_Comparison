function run_subject_level_analyses(raw_dir, preproc_dir, sub_template, unused, num_ignored_volumes, TR, varargin)
    ...
    for i = 1:numel(sub_dirs)    
        clearvars FUNC_RUN_* ONSETS_RUN_* ANAT FUNC_DIR OUT_DIR PREPROC_DIR BRAIN_EXTRACTED BRAIN_EXTRACTED_FILE
                
        [~,sub,~] = fileparts(sub_dirs{i});
        
        FUNC_DIR = fullfile(preproc_dir, sub, 'func');
        anat_dir = fullfile(preproc_dir, sub, 'anat');
        
        OUT_DIR = fullfile(preproc_dir, sub, 'level1');
